...
export default function DoctorProfilePage() {
  ...
  const submitReview = async () => {
    ...
    await supabase.from('reviews').insert({ doctor_id: doctor.id, patient_id: user.id, patient_name: profile?.full_name || 'مريض', rating: newRating, comment: newComment })
  ...
  const doctorImage = localImages[doctor.full_name] || doctor.avatar_url || `https://api.dicebear.com/7.x/initials/svg?seed=${doctor.full_name}&backgroundColor=0055A0&textColor=ffffff`
      ...
      <main className="section-padding">
        <div className="container mx-auto max-w-3xl">
          ...
          <div className="premium-card mb-8">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center sm:text-right">
                ...
                <p className="font-body text-primary flex items-center justify-center sm:justify-start gap-1 mt-1"><Stethoscope className="w-4 h-4" /> {doctor.specialization}</p>
                <div className="flex items-center justify-center sm:justify-start gap-1 mt-2">
                  {Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={`w-5 h-5 ${i < Math.round(doctor.rating || 0) ? 'text-yellow-400 fill-yellow-400' : 'text-muted'}`} />))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {doctor.bio && <div className="premium-card md:col-span-2"><h3 className="font-heading font-semibold mb-2">النبذة التعريفية</h3><p className="font-body text-muted-foreground text-sm">{doctor.bio}</p></div>}
            {doctor.phone && <div className="premium-card flex items-center gap-2"><Phone className="w-5 h-5 text-primary" /><span className="font-body text-sm">{doctor.phone}</span></div>}
            {(doctor.city || doctor.clinic_address) && <div className="premium-card flex items-center gap-2"><MapPin className="w-5 h-5 text-primary" /><span className="font-body text-sm">{[doctor.city, doctor.clinic_address].filter(Boolean).join(' - ')}</span></div>}
            {doctor.working_hours_start && <div className="premium-card flex items-center gap-2"><Clock className="w-5 h-5 text-primary" /><span className="font-body text-sm">{formatTime12(doctor.working_hours_start)} - {formatTime12(doctor.working_hours_end)}</span></div>}
            {doctor.diseases_treated?.length > 0 && (
              <div className="premium-card md:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">{doctor.diseases_treated.map((d: string) => (<span key={d} className="px-3 py-1 rounded-full bg-accent text-accent-foreground text-xs font-body">{d}</span>))}</div>
              </div>
            )}
          </div>

          <button onClick={handleBookClick} disabled={isDoctor} className="w-full py-4 rounded-xl bg-primary text-primary-foreground font-heading text-lg font-bold hover:bg-primary/90 transition-colors mb-8 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            {user && (
              <div className="premium-card mb-4">
                <div className="flex items-center gap-1 mb-3">
                  {Array.from({ length: 5 }).map((_, i) => (<button key={i} onClick={() => setNewRating(i + 1)}><Star className={`w-6 h-6 ${i < newRating ? 'text-yellow-400 fill-yellow-400' : 'text-muted'}`} /></button>))}
                </div>
                <textarea className="premium-input mb-3 min-h-[60px]" placeholder="اكتب تعليقك..." value={newComment} onChange={(e) => setNewComment(e.target.value)} />
                <button onClick={submitReview} disabled={submittingReview} className="px-6 py-2 rounded-xl bg-primary text-primary-foreground font-body text-sm hover:bg-primary/90 transition-colors disabled:opacity-50">
                ...
                {reviews.map((rev) => (
                  <div key={rev.id} className="premium-card">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex items-center gap-0.5">{Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={`w-3 h-3 ${i < rev.rating ? 'text-yellow-400 fill-yellow-400' : 'text-muted'}`} />))}</div>
                    </div>
