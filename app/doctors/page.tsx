...
import { getDemoCardData } from '@/lib/demoDoctors'
...
export default function DoctorsPage() {
  ...
  const loadDoctors = async () => {
    // أولاً: الأطباء التجريبيون دائماً موجودون
    const demoDoctors = getDemoCardData()

    // ثانياً: الأطباء من Supabase
    ...
    // دمج: Supabase أولاً ثم التجريبيون (بدون تكرار بناءً على الاسم)
    const supabaseNames = new Set(supabaseDoctors.map((d) => d.name))
    const merged = [
      ...supabaseDoctors,
      ...demoDoctors.filter((d) => !supabaseNames.has(d.name)),
    ]
        ...
        <div className="container mx-auto">
          ...
          <div className="max-w-2xl mx-auto mb-10 space-y-4">
            <div className="relative">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                placeholder="ابحث عن طبيب أو تخصص..." className="premium-input pr-12" />
            </div>
              ...
              {specializations.map((spec) => (
                <button key={spec} onClick={() => setFilter(spec)}
                  className={`px-4 py-2 rounded-xl font-body text-sm transition-all duration-200 ${
                    filter === spec ? 'bg-primary text-primary-foreground shadow-md' : 'bg-accent text-accent-foreground hover:bg-primary/10'
                  }`}>
                  {spec}
                </button>
              ))}
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16">
              <p className="font-body text-muted-foreground text-lg">لم يتم العثور على أطباء</p>
            </div>
          )}
        </div>
      </main>
